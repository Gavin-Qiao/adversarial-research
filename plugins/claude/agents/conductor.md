...
- Start with: "Before starting, run `pp codebook` to see existing artifacts."
- End with: "After completing, you MUST register your artifacts with `pp register ...`"
...
Follow the action. After dispatching an agent and saving its result, call `pp next` again to get the next action. Repeat until the state is `complete_*`.
...
```bash
pp extend-debate <claim-path> --to <N>
```
...
The protocol guides your judgment for **side-channel decisions** (mid-debate scout dispatches, early termination). The state machine (`pp next`) handles the **main control flow** (architect → adversary → experimenter → arbiter). When in doubt, follow `pp next`.

## Saving Results

After each agent dispatch, save the result to the file indicated by `result_path` from `pp next`. Standard locations:
...
When `pp next` returns `dispatch_arbiter`, YOU are the arbiter. Write your verdict to `{claim-path}/arbiter/results/verdict.md`.
